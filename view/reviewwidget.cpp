...
void ReviewWidget::create_widget()
{
    ...
    connect(m_playbackTimer, &QTimer::timeout, this, &ReviewWidget::onPlaybackTick);
    connect(dateEdit, &QDateEdit::dateChanged, this, &ReviewWidget::onFilterChanged);
    connect(channel_box, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &ReviewWidget::onFilterChanged);
    connect(btn_prev, &QPushButton::clicked, this, &ReviewWidget::prevPage);
    connect(btn_next, &QPushButton::clicked, this, &ReviewWidget::nextPage);
    connect(videos_list, &QListWidget::itemDoubleClicked,
            this, &ReviewWidget::onVideoItemDoubleClicked);
    connect(videos_list, &QListWidget::itemClicked,
            this, &ReviewWidget::onImageItemClicked);
    connect(btn_pause, &QPushButton::clicked, this, &ReviewWidget::togglePause);
    connect(speed_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &ReviewWidget::onSpeedChanged);
    connect(btn_screenshot, &QPushButton::clicked, this, &ReviewWidget::captureScreenshot);
    connect(progress_slider, &QSlider::sliderPressed, this, &ReviewWidget::onSliderPressed);
    connect(progress_slider, &QSlider::sliderReleased, this, &ReviewWidget::onSliderReleased);
    connect(progress_slider, &QSlider::sliderMoved, this, &ReviewWidget::onSliderMoved);
    connect(btn_image_query, &QPushButton::clicked, this, &ReviewWidget::toggleImageQuery);
    connect(btn_image_prev, &QPushButton::clicked, this, &ReviewWidget::imagePrevPage);
    connect(btn_image_next, &QPushButton::clicked, this, &ReviewWidget::imageNextPage);
}

void ReviewWidget::setup_left_panel(QWidget *panel)
{
    ...
    QLabel *lab_title = new QLabel("录像查询");
...
void ReviewWidget::setup_right_panel(QWidget *panel)
{
...
QWidget* ReviewWidget::create_image_page()
{
...
void ReviewWidget::toggleImageQuery()
{
    if (btn_image_query->isChecked()) {
        ...
        btn_image_query->setText("图片查询");
        right_stack->setCurrentIndex(0);
        updateVideoList();
    }
}

void ReviewWidget::onFilterChanged()
{
...
void ReviewWidget::updateImageList()
{
    current_image_page = 1;
    image_stack->setCurrentIndex(0);
    m_selectedImagePath.clear();
    ...
    int total = 0;
    fim.queryImages(channelId, queryDate, 0, image_page_size, 0, imageList, total);
...
void ReviewWidget::imagePrevPage()
{
    ...
    fim.queryImages(channelId, queryDate, 0, image_page_size, offset, imageList, total);
...
void ReviewWidget::imageNextPage()
{
...
void ReviewWidget::showImagesOnGrid(const QList<ImageInfo> &imageList)
{
...
void ReviewWidget::onImageItemClicked(QListWidgetItem *item)
{
...
void ReviewWidget::updateVideoList()
{
    current_page = 1;
    videos_list->clear();
    ...
    int total = 0;
    videoService.queryVideosByPage(channelId, queryDate, page_size, 0, videoList, total);
...
void ReviewWidget::prevPage()
{
    ...
    videoService.queryVideosByPage(channelId, queryDate, page_size, offset, videoList, total);
...
void ReviewWidget::onVideoItemDoubleClicked(QListWidgetItem *item)
{
    if (right_stack->currentIndex() == 1) return;
    stopPlayback();
...
void ReviewWidget::stopPlayback()
{
...
void ReviewWidget::onPlaybackTick()
{
...
void ReviewWidget::onSpeedChanged(int index)
{
    if (index < 0) return;
...
void ReviewWidget::captureScreenshot()
{
    if (!m_isPlaying) return;
    if (!m_videoCapture.isOpened()) return;
    ...
    QString filename = QString("review_screenshot_%1.png")
...
void ReviewWidget::onSliderPressed()
{
    if (!m_isPlaying) return;
    m_playbackTimer->stop();
}

void ReviewWidget::onSliderReleased()
{
    if (!m_isPlaying) return;
...
void ReviewWidget::onSliderMoved(int position)
{
    if (!m_isPlaying) return;
...
QImage ReviewWidget::MatToImage(const cv::Mat &frame)
{
    if (frame.empty()) {
        return QImage();
    }
